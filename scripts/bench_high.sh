...
mkdir -p ./metrics_data/fiber_high_deep ./metrics_data/fiber_high_flat ./metrics_data/gin_high_deep ./metrics_data/gin_high_flat
...
do
    ...
    savefile="fiber_high_deep"
    echo "$savefile"
    docker compose -f ./deployment/docker-compose.yml start fiber-app
    sleep 1
    start=$(date -u +%Y-%m-%dT%H:%M:%SZ)   
    pandora ./requests/pandora_config/deep_high.yaml > /dev/null 2>&1
    end=$(date -u +%Y-%m-%dT%H:%M:%SZ)
    make dump_logs
    mkdir -p "./metrics_data/${savefile}/${i}"                                          # mkdir -p "./metrics_data/fiber_high_deep/1"
    mv ./metrics_data/pandora_results/pandora.phout "./metrics_data/${savefile}/${i}"
    mv ./metrics_data/logs/logs_time_series.txt "./metrics_data/${savefile}/${i}"       # mv ./metrics_data/logs/logs_time_series.txt ./metrics_data/fiber_high_deep/1
    # end=$(date -u +%Y-%m-%dT%H:%M:%SZ)
    ...
    savefile="fiber_high_flat"
    echo "$savefile"
    docker compose -f ./deployment/docker-compose.yml start fiber-app
    sleep 1
    start=$(date -u +%Y-%m-%dT%H:%M:%SZ)    # В RFC3339 формате
    pandora ./requests/pandora_config/flat_high.yaml > /dev/null 2>&1
    end=$(date -u +%Y-%m-%dT%H:%M:%SZ)
    make dump_logs
